 ...
 *
 * 検証項目:
 *   [1] shinkansen → EX / えきねっと / JRネット予約 がラベルに存在する
...
/* ── [1] JRラベル新方式 ── */
check(linkBuilderSrc.includes('えきねっとで予約する'),   'えきねっとで予約する ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('EXで予約する'),           'EXで予約する ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('JRネット予約で予約する'), 'JRネット予約で予約する ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('JR九州ネット予約で予約する'), 'JR九州ネット予約で予約する ラベルが存在しない', 'linkBuilder.js');

/* ── [2] IC在来線の注記 ── */
check(rendererSrc.includes('ICカードでそのまま乗車できます'), 'IC在来線注記テキストが存在しない', 'transportRenderer.js');
...
check(renderSrc.includes('btn--step-main'),   'render.js にメインCTAボタンクラスが存在しない', 'render.js');
...
check(styleSrc.includes('.btn--step-main'),   'style.css に .btn--step-main が存在しない', 'style.css');
check(styleSrc.includes('.step-card-caution'), 'style.css に .step-card-caution が存在しない', 'style.css');

/* ── [7] hotelLinkBuilder ── */
check(hotelSrc.includes('hotel/search.do'), '楽天がhotel検索URLを使っていない（pack禁止）', 'hotelLinkBuilder.js');
...
check(
  (hotelSrc.match(/encodeURIComponent/g) || []).length <= 3,
  'じゃらんに多重encode（encodeURIComponent）の疑いがある',
  'hotelLinkBuilder.js',
);
check(
  hotelSrc.includes('dest.hotelSearch || dest.displayName || dest.name'),
  'hotelLinkBuilder のキーワード解決ロジックが変更されている',
  'hotelLinkBuilder.js',
);
