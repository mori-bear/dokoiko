...
/* ── [1] JRラベル新方式 ── */
check(linkBuilderSrc.includes('最短ルートで予約する（えきねっと）'),     '最短ルートで予約する（えきねっと） ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('最短ルートで予約する（EX）'),             '最短ルートで予約する（EX） ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('最短ルートで予約する（e5489）'),           '最短ルートで予約する（e5489） ラベルが存在しない', 'linkBuilder.js');
check(linkBuilderSrc.includes('最短ルートで予約する（JR九州ネット予約）'), '最短ルートで予約する（JR九州ネット予約） ラベルが存在しない', 'linkBuilder.js');
...
check(
  ...
  '楽天が search?keyword= も hotel/search.do も使っていない（pack禁止）',
  'hotelLinkBuilder.js',
);
