// 既知 Vision NG 9 件 を Pixabay で再取得 (md5重複回避)
...
const ids = JSON.parse(fs.readFileSync(path.join(ROOT, 'logs/visionNGIds.json'), 'utf-8'));
...
for(const id of ids){
    ...
    for(const page of [1,2,3,4]){
      ...
      for(const url of hits){
        try{
          ...
          usedMd5.add(h);
          placed={url,size:buf.length,q,page};
          break;
        }catch{}
      }
      if(placed) break;
  ...
  if(placed){ok++; console.log('OK',d.id,'->',placed.size,'bytes via',placed.q,'p'+placed.page);}
