 ...
 * ここに登録された ID は JR main-cta を生成せず、代わりに Google Maps main-cta を返す。
 */
...
/**
 * EXCLUDE_FROM_JR_CTA 該当時に Google Maps transit main-cta を組み立てる。
 */
function _buildGoogleMapsMainCta(city, departure) {
  const fromStation = DEPARTURE_CITY_INFO[departure]?.rail ?? departure;
  const toStation   = city?.railGateway ?? city?.displayName ?? city?.name;
  const url = buildGoogleMapsLink(fromStation, toStation, 'transit');
  if (!url) return null;
  return {
    type: 'google-maps',
    url,
    label: 'Google Mapsで行き方を調べる',
  };
}
...
function _deriveMainCtaFromSteps(steps, departure, city) {
  ...
  // EXCLUDE_FROM_JR_CTA 該当（私鉄専用駅）→ JR CTA を出さず Google Maps で代替
  const isJrExcluded  = EXCLUDE_FROM_JR_CTA.has(city?.id);
  const fallbackRail  = () => {
    if (isOkinawa) return null;
    if (isJrExcluded) return _buildGoogleMapsMainCta(city, departure);
  ...
  switch (primaryStep.type) {
    ...
    case 'rail': {
      ...
      // EXCLUDE_FROM_JR_CTA 該当（私鉄専用駅）→ Google Maps main-cta
      if (isJrExcluded) return _buildGoogleMapsMainCta(city, departure);
      // 通常の鉄道 → JR リンク生成
...
function _deriveMainCtaFromStepGroups(stepGroups, city, departure) {
  ...
  // EXCLUDE_FROM_JR_CTA 該当（私鉄専用駅）→ Google Maps main-cta
  if (EXCLUDE_FROM_JR_CTA.has(city?.id)) return _buildGoogleMapsMainCta(city, departure);
  return buildJrLink(_deriveJrProviderFromStep(null, departure));
}
